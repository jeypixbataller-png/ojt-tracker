...
export default function NotificationBell({ announcements }) {
  ...
  const dropdown = open ? createPortal(
    <>
      <div onClick={() => setOpen(false)} style={{
        ...
        background: window.innerWidth <= 768 ? 'rgba(0,0,0,.3)' : 'transparent',
      }} />
      <div ref={dropRef} style={{
        ...
        boxShadow: '0 16px 48px rgba(0,0,0,.2), 0 4px 12px rgba(0,0,0,.08)',
  ...
  return (
    <>
      <button ref={btnRef} className="btn btn-ghost btn-icon" onClick={toggleOpen} style={{ position: 'relative', width: 36, height: 36, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Bell size={18} />
        {unread.length > 0 && (
          <span style={{
